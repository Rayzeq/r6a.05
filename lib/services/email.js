...
module.exports = class EmailService extends Service {
    ...
    async sendCSVExportEmail(recipientEmail, csvContent, fileName) {
        const html = `
            <h1>Export de la bibliothèque</h1>
            <p>Veuillez trouver ci-joint l'export de la bibliothèque</p>
        `;

        const mailOptions = {
            from: process.env.SMTP_FROM || '"Projet JS" <noreply@projet-js.com>',
            to: recipientEmail,
            subject: 'Export de la bibliothèque',
            html,
            attachments: [
                {
                    filename: fileName,
        ...
        try {
            ...
            console.error('Failed to send CSV export email:', error);
            throw error;
        }
    }
};
